...
export default function Home() {
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-transparent text-[var(--foreground)]">
      ...
      <main>
        <section className="relative min-h-screen overflow-hidden px-5 pb-12 pt-28 sm:px-10 lg:px-16">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_55%_35%,rgba(255,253,248,0.08),rgba(247,244,238,0.56)_68%,rgba(247,244,238,0.76)_100%)]" />
          <div className="relative z-10 mx-auto grid min-h-[calc(100vh-9rem)] max-w-7xl items-end gap-12 lg:grid-cols-[1.05fr_0.95fr]">
            <div className="pb-20">
              ...
              <div className="mt-10 flex flex-wrap gap-3">
                <a
                  href="#proposal"
                  className="border border-[var(--foreground)] bg-[var(--foreground)] px-5 py-3 text-sm font-black uppercase text-[var(--paper)] transition hover:bg-transparent hover:text-[var(--foreground)]"
                >
                  {content.home.hero.primaryCta}
                </a>
                <Link
                  href="/work"
                  className="border border-[var(--line)] px-5 py-3 text-sm font-black uppercase transition hover:border-[var(--foreground)]"
                >
        ...
        <section className="px-5 py-16 sm:px-10 lg:px-16">
          <div className="mx-auto grid max-w-7xl border-y border-[var(--line)] md:grid-cols-3">
            {content.home.metrics.map((metric) => (
              <div
                key={metric.label}
                className="border-[var(--line)] py-8 md:border-r md:px-8 md:last:border-r-0"
              >
        ...
        <section className="px-5 py-12 sm:px-10 lg:px-16">
          <div className="mx-auto grid max-w-7xl gap-4 md:grid-cols-3">
            {content.services.map((service, index) => (
              <article
                key={service.slug}
                className="group border border-[var(--line)] bg-[var(--paper-soft)] p-5 transition hover:-translate-y-1 hover:border-[var(--foreground)]"
              >
                ...
                />
                <p className="mt-5 text-base font-medium leading-tight text-[var(--muted)]">
                  {service.text}
                </p>
              </article>
            ))}
          ...
          <div className="mx-auto max-w-7xl">
            <div className="grid gap-3 md:grid-cols-4">
              {content.home.process.map((step, index) => (
                <article
                  key={step.title}
                  className="technical-grid border border-[var(--line)] bg-[var(--paper-soft)] p-5"
                >
            ...
            <div className="mt-10 grid gap-5 md:grid-cols-3">
              {content.projects.map((project) => (
                <Link
                  key={project.title}
                  href={project.href}
                  className="group border border-[var(--line)] bg-[var(--paper-soft)] p-4 transition hover:border-[var(--foreground)]"
                >
                  <Image
                    src={project.image}
                  ...
                  <div className="mt-5 flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-1 font-mono text-xs font-bold uppercase tracking-[0.16em] text-[var(--muted)]">
                        {project.category} / {project.year}
                      </p>
                    </div>
                    <span className="text-sm font-black text-[var(--pulse)]">
                      {project.status}
                    </span>
                  </div>
                </Link>
