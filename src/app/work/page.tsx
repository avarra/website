    ...
    <div className="min-h-screen bg-transparent text-ink">
      ...
      <main className="overflow-hidden">
        ...
        <section className="px-5 py-20 sm:px-10 lg:px-16">
          <div className="mx-auto max-w-7xl">
            <div className="flex items-end justify-between gap-6 border-b border-line pb-6">
              <h1 className="text-[clamp(3rem,8vw,8rem)] font-extrabold leading-[0.85]">
                {content.workPage.projectsHeading}
              </h1>
              <p className="hidden max-w-xs text-right font-mono text-xs font-semibold uppercase tracking-[0.2em] text-muted md:block">
            ...
            <div className="mt-8 grid gap-5 md:grid-cols-3">
              {content.projects.map((project) => (
                <article
                  key={project.title}
                  className="group border border-line bg-white p-4"
                >
                  ...
                  <div className="mt-5 flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-2 font-mono text-xs font-semibold uppercase tracking-[0.16em] text-muted">
                        {project.category} / {project.year}
                      </p>
                    </div>
                    <span className="border border-line px-2 py-1 font-mono text-[10px] font-semibold uppercase tracking-[0.12em] text-brand">
                      {project.status}
                    </span>
                  </div>
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    {project.tags.map((tag) => (
                      <span
                        key={tag}
                        className="bg-paper-muted px-2 py-1 font-mono text-[10px] font-semibold uppercase tracking-[0.14em]"
                      >
                        {tag}
                      </span>
                    ))}
          ...
          <div className="mx-auto grid max-w-7xl gap-6 border-y border-line py-10 md:grid-cols-[0.8fr_1.2fr]">
            ...
            <div className="flex flex-wrap gap-2 md:justify-end">
              {content.partners.map((partner) => (
                <span
                  key={partner}
                  className="border border-line bg-white px-3 py-2 font-mono text-xs font-semibold uppercase tracking-[0.18em]"
                >
                  {partner}
                </span>
              ))}
        ...
        <section className="px-5 py-16 sm:px-10 lg:px-16">
          <div className="mx-auto max-w-7xl">
            <h2 className="text-[clamp(2.7rem,6vw,6rem)] font-extrabold leading-[0.9]">
            ...
            <div className="mt-8 grid gap-3 md:grid-cols-3">
              {content.buildLog.map((item, index) => (
                <article
                  key={item.title}
                  className="signal-scan border border-line bg-white p-5"
                >
                  <p className="font-mono text-xs font-semibold uppercase tracking-[0.22em] text-brand">
                    0{index + 1}
                  </p>
                  <h3 className="mt-12 text-3xl font-extrabold leading-none">
                    {item.title}
                  </h3>
